 ...
 */
function initializePrePublishValidation(): void {
	...
	validationInstance.registerRule( 'excerptLength', ( post: any ) => {
		...
		const editorStore = select( 'core/editor' ) as any;
		const excerpt = editorStore
			? editorStore.getEditedPostAttribute( 'excerpt' )
			: post.excerpt?.raw || '';
...
domReady( initializePrePublishValidation );
